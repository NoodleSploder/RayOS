...
impl IntentParser {
    ...
    pub fn parse(&mut self, input: &str, context: Context) -> ParseResult {
        ...
        // First: short-circuit purely conversational inputs into a "chat" command.
        // This prevents things like "what are you doing" from being treated as a repo Search.
        if is_chat_like(&input) {
            let intent = Intent {
                id: IntentId::new(),
                command: Command::Execute {
                    action: "chat".to_string(),
                    args: vec![raw_input.to_string()],
                },
...
fn is_chat_like(input_lower: &str) -> bool {
    let s = input_lower.trim();
    if s.is_empty() {
        return true;
    }

    // If the user is explicitly issuing a task-like command, don't hijack it.
    // (They can still be frustrated *and* give a command; the command should win.)
    if s.starts_with("search ")
        || s.starts_with("find ")
        || s.starts_with("index ")
        || s.starts_with("optimize ")
        || s.starts_with("run ")
        || s.starts_with("execute ")
    {
        return false;
    }
    ...
    if matches!(first, "hi" | "hello" | "hey" | "yo" | "sup" | "hiya" | "howdy" | "ping") {
        return true;
    }
    if matches!(first, "thanks" | "thank" | "thx" | "ty") {
        return true;
    }
    if matches!(first, "bye" | "goodbye" | "cya") {
        return true;
    }

    // Status/help/identity questions.
    if s.contains("help") || s.contains("status") {
        return true;
    }
    if s.contains("who are you") || s.contains("what are you") || s.contains("what are you doing") {
        return true;
    }

    // Frustration / bug-report tone often needs dialog, not task execution.
    // Keep this intentionally narrow.
    if s.contains("sucks")
        || s.contains("wtf")
        || s.contains("broken")
        || s.contains("doesn't work")
        || s.contains("doesnt work")
...
mod tests {
    ...
    #[test]
    fn test_parse_chat_like_question() {
        let mut parser = IntentParser::new(IntentConfig::default());
        let result = parser.parse("what are you doing", make_context());

        match result.intent.command {
            Command::Execute { ref action, .. } => assert_eq!(action, "chat"),
            other => panic!("Expected chat Execute command, got: {:?}", other),
        }
        assert!(!result.needs_clarification);
    }

    #[test]
    fn test_parse_frustration_is_chat() {
        let mut parser = IntentParser::new(IntentConfig::default());
        let result = parser.parse("this sucks", make_context());
