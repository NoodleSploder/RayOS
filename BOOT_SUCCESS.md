...
**File Location**: `iso-output/rayos.iso` (7.88 MB)
...
# ISO output at: iso-output/rayos.iso
```
...
Add-VMDvdDrive -VMName RayOS -Path "$(Get-Location)\iso-output\rayos.iso"
Start-VM -Name RayOS
```
...
2. Select ISO: `iso-output/rayos.iso`
3. Settings:
...
2. **Verify ISO Integrity**:
   ...
   ```powershell
   Get-Item iso-output\rayos.iso | Select-Object Length
...
- ✅ `/iso-output/rayos.iso` - Generated bootable ISO
