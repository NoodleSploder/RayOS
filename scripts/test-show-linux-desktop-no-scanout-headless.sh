#!/bin/bash
# Headless validation for "show linux desktop" when no guest scanout exists.
#
# Boots kernel-bare under OVMF (without dev_scanout), injects "show linu desktop"
# via QEMU monitor sendkey, and verifies the kernel prints guidance that no guest
# scanout is available.
...
STAGE_DIR="${STAGE_DIR:-$WORK_DIR/no-scanout-fat}"
...
SERIAL_LOG="${SERIAL_LOG:-$WORK_DIR/serial-no-scanout.log}"
QEMU_LOG="${QEMU_LOG:-$WORK_DIR/qemu-no-scanout.log}"
MON_LOG="${MON_LOG:-$WORK_DIR/monitor-no-scanout.log}"
MON_SOCK="${MON_SOCK:-$WORK_DIR/qemu-monitor-no-scanout.sock}"
PID_FILE="${PID_FILE:-$WORK_DIR/qemu-no-scanout.pid}"
...
# Build kernel WITHOUT dev_scanout.
...
HINT_MARKER="SYS: no guest scanout available; run RAYOS_KERNEL_FEATURES=dev_scanout"
...
if ! wait_for_log "$HINT_MARKER" "$TIMEOUT_SECS"; then
  echo "FAIL: missing no-scanout hint marker" >&2
  tail -n 200 "$SERIAL_LOG" 2>/dev/null || true
  exit 1
fi

echo "PASS: no-scanout guidance observed"
