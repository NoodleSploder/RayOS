#!/usr/bin/env bash
# Headless aarch64 kernel boot smoke test (Option B bring-up).
#
# Stages an ESP FAT dir with EFI\\RAYOS\\kernel.bin set to the ELF produced by
# kernel-aarch64-bare, then boots under AAVMF and verifies we reach the kernel.
...
STAGE_DIR="${STAGE_DIR:-$WORK_DIR/aarch64-kernel-headless-fat}"
OUT_LOG="${OUT_LOG:-$WORK_DIR/serial-boot-aarch64-kernel-headless.log}"
...
VARS_FD="/tmp/rayos-aavmf-vars-kernel-headless.fd"
cp -f "$AAVMF_VARS_SRC" "$VARS_FD"

rm -f "$OUT_LOG" 2>/dev/null || true

echo "Starting RayOS aarch64 kernel headless test..." >&2
...
if ! grep -q "RayOS: GPU probe: GOP handles" "$OUT_LOG"; then
  echo "FAIL: did not see bootloader GPU probe GOP handles marker" >&2
  echo "Serial log: $OUT_LOG" >&2
  exit 1
fi
if ! grep -q "RayOS: GPU probe: fb_base=" "$OUT_LOG"; then
  echo "FAIL: did not see bootloader GPU probe framebuffer base marker" >&2
  echo "Serial log: $OUT_LOG" >&2
  exit 1
fi
...
# Optional sanity: kernel should emit at least one bridge line in its loop.
if ! grep -q "RAYOS_INPUT:" "$OUT_LOG"; then
  echo "FAIL: did not observe any RAYOS_INPUT lines (UART path may be broken)" >&2
  echo "Serial log: $OUT_LOG" >&2
  exit 1
fi

echo "PASS: reached kernel-aarch64-bare entry" >&2
