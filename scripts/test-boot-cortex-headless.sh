#!/bin/bash
# Headless Cortex->Kernel protocol smoke test (x86_64 kernel-bare).
# - Boots kernel-bare under OVMF
# - Injects a Cortex INTENT event via the shell passthrough (:cortex ...)
# - Verifies kernel logs the event and forwards into System 2
...
STAGE_DIR="${STAGE_DIR:-$WORK_DIR/cortex-headless-fat}"
MON_SOCK="${MON_SOCK:-$WORK_DIR/qemu-monitor-cortex-headless.sock}"
LOG_FILE="${LOG_FILE:-$WORK_DIR/cortex-headless.log}"
PID_FILE="${PID_FILE:-$WORK_DIR/cortex-headless.pid}"
...
"$QEMU_BIN" \
  ...
  -no-reboot \
  -net none \
  >"$WORK_DIR/qemu-cortex-headless.log" 2>&1 &
...
# Inject Cortex line via shell passthrough and quit.
python3 "$ROOT_DIR/scripts/qemu-sendtext.py" --sock "$MON_SOCK" --text ":cortex $CORTEX_LINE" --after 0.8 --quit
...
# Normalize CRLF.
NORM="$WORK_DIR/cortex-headless.norm.log"
...
echo "OK: Cortex->Kernel headless smoke test passed. Log: $LOG_FILE" >&2
