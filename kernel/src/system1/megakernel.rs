...
fn megakernel_main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Each thread runs an infinite loop (in practice, bounded by dispatch)
    // This would ideally be while(true) if GPUs supported it
    for (var iteration = 0u; iteration < 1000u; iteration++) {
...
fn execute_ray(ray: LogicRay, thread_id: u32) {
    ...
    var current_node = ray.logic_tree;
    var depth = 0u;
    let max_depth = 15u;
    ...
    loop {
        ...
        let hit = (ray.task_id + depth) % 3u != 0u;  // 2/3 hit rate
    ...
    let output_idx = ray.task_id & 0xFFFFu;
