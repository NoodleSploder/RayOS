...
impl Watcher {
    ...
    pub fn update_entropy(&mut self, metrics: &SystemMetrics) {
        // Simple entropy calculation: ratio of latency to target
        let target_latency = 16_666; // 60 FPS target
        self.entropy = (metrics.avg_latency_us as f32 / target_latency as f32).min(1.0);
    }

    /// Check if we should enter dream mode
    pub fn should_dream(&self, config: &KernelConfig) -> bool {
        if !config.enable_dream_mode {
            return false;
        }

        let idle_time = self.last_interaction.elapsed().as_secs();
