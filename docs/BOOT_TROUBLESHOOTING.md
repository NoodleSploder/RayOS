...
3. In Rufus:
   ...
   - **Boot selection**: Click "SELECT" → choose `build/rayos.iso`
...
if (Test-Path $VBoxPath) {
   ...
   & $VBoxPath storageattach RayOS-Test --storagectl IDE --port 0 --device 0 --type dvddrive --medium "$(Get-Location)\build\rayos.iso"
...
# List ISO contents
wsl 'iso-content=$(mktemp -d) && mount -o loop build/rayos.iso $iso-content && find $iso-content -type f && umount $iso-content'

# Or verify EFI binary exists in ISO
wsl tar -tzf build/rayos.iso | grep -i boot
```
