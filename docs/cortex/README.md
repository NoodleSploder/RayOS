...
- Graceful shutdown
- Kernel communication protocol (line-based `CORTEX:<TYPE> k=v ...`) and x86_64 `kernel-bare` receiver

### ✅ Kernel-bare integration (headless QEMU)

For x86_64 `kernel-bare` development, Cortex can inject `CORTEX:` messages into a running QEMU guest via the QEMU monitor socket (uses `sendkey` under the hood).

This is primarily for automated smoke tests and bring-up.

**One-shot send mode (no camera/OpenCV required)**:
...
- Richer transport options beyond QEMU monitor injection (e.g. virtio/serial bridge)
...
### Hardware eye tracking (UDP gaze bridge)

Cortex supports a simple hardware eye-tracker input path over UDP. This is intended to be fed by an external bridge (for Tobii or other trackers) without linking proprietary SDKs into RayOS.

- Set `RAYOS_GAZE_UDP_ADDR` to a bind address (example: `127.0.0.1:5555`)
- Send UDP datagrams as either:
    - JSON: `{"x":0.5,"y":0.5,"confidence":1.0,"timestamp":123}`
    - Or tokens: `x=0.5 y=0.5 conf=1.0 ts=123`

When UDP gaze is enabled, it overrides the webcam-based gaze estimation.
...
### Real object recognition (OpenCV DNN)

Object recognition can run a real OpenCV DNN model (optional). Provide MobileNet-SSD Caffe model paths via env vars:

- `RAYOS_DNN_SSD_PROTOTXT=/path/to/deploy.prototxt`
- `RAYOS_DNN_SSD_MODEL=/path/to/weights.caffemodel`
- Optional: `RAYOS_DNN_CLASSES=/path/to/classes.txt`
- Optional: `RAYOS_DNN_INPUT=300`

If these aren’t set (or files are missing), Cortex falls back to the existing deterministic simulated detections.
