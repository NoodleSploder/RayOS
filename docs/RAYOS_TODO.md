...
- TODOs:
	...
	- Prerequisites (blockers):
		...
		- ✅ RayOS compositor/presentation can ingest guest scanout buffers (kernel-bare `guest_surface` publish/snapshot + native blit path exist; now exercised by virtio-gpu scanout publish).
	...
	**Hypervisor track: fastest path to “Linux Subsystem under RayOS VMM” (execution checklist)**
	...
	P1 (Must-have): boot Linux headless under RayOS VMM — status: **done** (core gates)
	...
	- [x] Virtio-blk backing (READ/WRITE/GET_ID) — works with the deterministic guest blob; now supports **image-backed initialization** via feature `vmm_virtio_blk_image` (image generated by `scripts/generate-vmm-disk-image.sh`).
	- [~] Virtio-console or minimal serial transport for guest logs/markers — **basic queue parsing implemented**: data/control queues handled, host logging emitted (markers: `RAYOS_VMM:VIRTIO_CONSOLE:COMPILED` / `ENABLED` / `CHAIN_HANDLED` / `RECV`, control markers `CTRLQ_HANDLED` present). Added a boot-time selftest feature (`vmm_virtio_console_selftest`) that exercises the handler during boot and emits `RAYOS_VMM:VIRTIO_CONSOLE:SELFTEST:INVOKE`. Added deterministic guest-driven test blob generation (`scripts/generate_guest_driver.rs` with `RAYOS_GUEST_CONSOLE_ENABLED=1`) and smoke-script validation. Remaining: richer control semantics and protocol conformance verification.
		- Verification (standalone): `./scripts/test-vmm-virtio-console-headless.sh` (guest-driven dataq message; asserts `RAYOS_VMM:VIRTIO_CONSOLE:RECV` when VMX reaches VMCS_READY).
	- [✅] Linux boots to a deterministic “guest ready” marker under RayOS VMM (headless harness asserts readiness via serial markers).
		- Marker emission: guest init (`/rayos_init`) emits `RAYOS_LINUX_GUEST_READY` exactly once.
		- Robustness: marker is written to `/dev/kmsg` (and also attempts `/dev/console`/`/dev/ttyS0`) so it is visible even if early `/dev/console` plumbing is missing.
	...
	**Notes / Implementation highlights:**
	...
	- [✅] Wire virtio-gpu device model to the *real* virtqueue transport (controlq + cursorq if needed; start with scanout-only).
		- Implemented: virtio-gpu controlq dispatch supports GPA→HPA translation so the device model reads requests correctly and publishes CPU-visible scanout backing.
	- [✅] Choose scanout backing (start CPU-visible shared backing: guest writes, RayOS blits).
		- Implemented: initial path uses host CPU-visible backing via translated guest RAM pages.
	...
	- [✅] Add a headless test for a *real Linux desktop* presented via in-OS virtio-gpu scanout (no host bridge).
		- Script: `./scripts/test-vmm-linux-desktop-native-present-headless.sh` (asserts `RAYOS_LINUX_DESKTOP_PRESENTED` and `RAYOS_LINUX_DESKTOP_FIRST_FRAME`).
