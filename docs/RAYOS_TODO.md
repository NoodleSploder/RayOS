...
- TODOs:
	...
	**Hypervisor track: fastest path to “Linux Subsystem under RayOS VMM” (execution checklist)**
	...
	- [~] Virtio-console or minimal serial transport for guest logs/markers — **scaffolded**: dispatch + basic host logging implemented (markers: `RAYOS_VMM:VIRTIO_CONSOLE:COMPILED` / `ENABLED` / `CHAIN_HANDLED` / `RECV`). Added a boot-time selftest feature (`vmm_virtio_console_selftest`) that exercises the handler during boot and emits `RAYOS_VMM:VIRTIO_CONSOLE:SELFTEST:INVOKE`. Remaining: queue parsing refinements, control/data semantics, and guest-side test cases.
